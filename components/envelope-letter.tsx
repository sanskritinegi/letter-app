"use client"
import type { Recipient } from "@/lib/letters"
...
export default function EnvelopeLetter({ recipient }: EnvelopeLetterProps) {
  ...
  const formatAddress = (address: Recipient["address"]) => {
    return [address.line1, address.line2, `${address.city}, ${address.state} ${address.zip}`, address.country].filter(
      Boolean,
    )
  }
          ...
          >
            ...
            <div className="absolute inset-0 bg-[#f8f8f8] rounded-lg border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
              ...
              <div className="absolute top-6 left-6 font-mono text-[#2d2b6e] text-sm">
                <p className="font-bold">{recipient.sender.name}</p>
                {formatAddress(recipient.sender.address).map((line, index) => (
                  <p key={index}>{line}</p>
                ))}
              </div>
              ...
              <div className="absolute bottom-6 right-6 font-mono text-[#9c2b7a] text-sm text-right">
                ...
                {formatAddress(recipient.address).map((line, index) => (
                  <p key={index}>{line}</p>
                ))}
              </div>
      ...
      >
        ...
        <div className="flex flex-col p-6 pt-8 font-handwriting text-[#2c3e50] h-full overflow-hidden">
          <div className="text-right mb-4">
            <p>{recipient.letter.date}</p>
          </div>

          <div className="mb-3">
            <p className="text-lg">{recipient.letter.greeting}</p>
          </div>
          ...
          <div ref={contentRef} className="space-y-4 leading-6 text-base flex-1 overflow-y-auto pr-2 relative">
            {recipient.letter.paragraphs.map((paragraph, index) => (
              <p key={index}>{paragraph}</p>
            ))}

            <div className="mt-4 mb-4">
              <p className="mb-1">{recipient.letter.closing}</p>
              <p className="italic text-lg">{recipient.letter.signature}</p>
            </div>
          </div>
        </div>
